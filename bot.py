...
def topic_questions(code: str) -> str:
    ...
    if code == "IDEA":
        return (
            "Опишите идею/улучшение:\n"
            "• Что предлагаете добавить/изменить?\n"
    ...
    if code == "AUTH":
        return (
            "Опишите проблему со входом/аккаунтом:\n"
...
async def save_details(message: Message, state: FSMContext):
    ...
    if not text:
        await message.answer("Пожалуйста, отправьте текстом описание (одним сообщением).")
        return

    await state.update_data(details=text)
    await state.set_state(TicketFlow.attach)
    await message.answer("Хотите прикрепить скрин/видео/файл?", reply_markup=yes_no_keyboard())
...
async def attach_choice(call: CallbackQuery, state: FSMContext):
    ...
    if choice == "yes":
        await call.message.answer("Ок! Пришлите следующим сообщением скрин/видео/файл. Можно 1 файл.")
        # ждём вложение
        await state.set_state(State("await_attachment"))
    else:
        await call.message.answer("✅ Принято! Мы ответим вам скоро.")
        await state.clear()
...
async def handle_attachment(message: Message, state: FSMContext):
    ...
    await message.answer("✅ Вложение отправлено. Спасибо! Мы ответим вам скоро.")
    await state.clear()
